...
export default function Services() {
  ...
  const handleWhatsApp = (msg) => {
    window.open(`https://wa.me/${brand.whatsapp}?text=${encodeURIComponent(formatLeadText(msg))}`, '_blank');
  };

  const handleEmail = (subject, body) => {
    window.open(`mailto:${brand.email}?subject=${encodeURIComponent(formatLeadText(subject))}&body=${encodeURIComponent(formatLeadText(body))}`, '_blank');
  };
  ...
  return (
    <section id="services" className="py-20 md:py-28 bg-white relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-[0.02]" style={{
        backgroundImage: `repeating-linear-gradient(45deg, #070D22 0px, #070D22 1px, transparent 0px, transparent 50%)`,
              ...
              >
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-cyan/10 hover:bg-cyan text-cyan hover:text-white text-xs font-body font-semibold transition-all duration-200 border border-cyan/10 hover:border-cyan"
